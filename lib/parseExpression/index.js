...
module.exports = function parseExpression(expression, now) {
  ...
  if (~expression.indexOf('.') && !~expression.indexOf(' ')) {
    var parts = expression.split('.').map(function (x) {
      return +x;
    });
    if (parts.length === 1) return { major: parts[0] };
    else if (parts.length === 2) return { major: parts[0], minor: parts[1] };
    else return { major: parts[0], minor: parts[1], patch: parts[2] };
  }
  if (!isNaN(+expression)) return { major: +expression };
  if (expression === 'lts') {
    var lts = schedulesLatest(this.schedules, isLTSFn(now));
    return lts ? { schedule: lts.name } : null;
  }
  if (expression === 'latest') {
    var latest = schedulesLatest(this.schedules, isLatestFn(now));
    return latest ? { schedule: latest.name } : null;
  }
