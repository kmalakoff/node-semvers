...
describe('cli', (_done) => {
  ...
  describe('happy path', (_done) => {
    it('12.14.0', (done) => {
      spawn(CLI, ['12.14.0', '--now', now.getTime()], { encoding: 'utf8' }, (err, res) => {
        const stdout = res ? res.stdout : err.stdout;
        const version = cr(stdout).split('versions:\n').pop().split('\n')[0];
    ...
    it('stable (path: raw)', (done) => {
      spawn(CLI, ['stable', '--now', now.getTime(), '--path', 'raw'], { encoding: 'utf8' }, (err, res) => {
        const stdout = res ? res.stdout : err.stdout;
        const version = JSON.parse(cr(stdout).split('versions:\n').pop().split('\n')[0]);
  ...
  describe('happy path range', (_done) => {
    it('10.x || >=12.0.0', (done) => {
      spawn(CLI, ['10.0.0 || ~12.0.0', '--now', now.getTime()], { encoding: 'utf8' }, (err, res) => {
        const stdout = res ? res.stdout : err.stdout;
        const versions = cr(stdout).split('versions:\n').pop().split('\n').slice(0, -1);
